Feature: Generating a new Copier template
  ...
  Scenario Outline: Generating a new Copier template with the custom License Owner
    ...
    Then the contents of the ".tmp/custom-license-owner-<Dir Name>" directory should be the same as the contents in the "features/fixtures/copier-templates/custom-license-owner-<License Owner>" directory with excludes
      | Exclude                       |
      | .release-please-manifest.json |
      | .tmp                          |
      | CHANGELOG.md                  |
